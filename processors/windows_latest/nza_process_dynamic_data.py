...
#                                                                             *
#   DESCRIPTION                                                               *
#   Cleans and reformats the raw, DYNAMIC data by aggregating and filling     *
...
# Example usage with different formats, including pandas-style datetime
# if __name__ == "__main__":
#     date_inputs = [
#         "2024-02-15",
#         "15-02-2024",
#         "15/02/2024",
#         "24-01-01 08:00:00",  # Pandas datetime format
#         "2024-01-01 08:00:00",
#         "2024-01-01 08:00",
#         "February 15, 2024",
#         "Invalid Date String"
...
#     for date_string in date_inputs:
#         try:
#             print(extract_detailed_date_info(date_string))
...
if __name__ == '__main__':
    ...
    config_file = '/home/pbrun/pypsa-nza/config/nza_raw_dyn_data.yaml'
    ...
    grid_energy_types = config['grid_energy_types']
    scale_factor = 1.0e-3   # Scale factor to convert from kWh to MWh

    # Loop over energy types cleaning the data
    for grid_energy in grid_energy_types:
        inp = root + config['paths'][f'{grid_energy}_in']
        out = root + config['paths'][f'{grid_energy}_out']
        file_tag = f"{grid_energy}_md"

        ensure_directory_exists(out, verbose = False)

        print(f"\n{grid_energy}".upper())
        energy_by_modality(inp, out, file_tag)


    # gen_in = root + config['paths']['gen_in']
    # gen_out = root + config['paths']['gen_out']
    # ensure_directory_exists(gen_out, verbose = True)

    # export_in = root + config['paths']['export_in']
    # export_out = root + config['paths']['export_out']
    # ensure_directory_exists(export_out, verbose = True)

    # import_in = root + config['paths']['import_in']
    # import_out = root + config['paths']['import_out']
    # ensure_directory_exists(import_out, verbose = True)

    # hvdc_in = root + config['paths']['hvdc_in']
    # hvdc_out = root + config['paths']['hvdc_out']
    # ensure_directory_exists(import_out, verbose = True)
