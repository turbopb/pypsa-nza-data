...
"""
Grid Get Raw Static Data Processor

Preprocesses, filters and reformats static raw data pertaining to the
NZ power network. Static data includes things like fixed assets, infrastructure
specifications, geographic locations, etc. which change infrequently (< 12 months).
...
The cleaned data is written to the processed data directory for PyPSA model input.

Author: Phillippe Bruneau
Created: Mon July 01 14:13:38 2024
Updated: Sat Sep  20 19:33:57 2025
...
from python_yaml import load_config
...
class POCProcessor:
    ...
    def _finalize_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        """Perform final cleanup and column reordering."""
...
def main():
    ...
    try:
        # Load configuration
        config = load_config("../../configs/clean_raw_data.yaml")
